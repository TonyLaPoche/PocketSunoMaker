...
- [ ] Effets (glitch, rotation, etc.) pluginables
  - Progression v1: boite a outils Effets avec 2 categories (visuels/sonores), ajout d effets sur timelines dediees multi-pistes, premiers effets visuels Filmora-like en preview (glitch, tremblement, RGB split, flash, VHS), premiers effets sonores placeholders (bip censure, distorsion, stutter) prets pour la phase de rendu audio, export FFmpeg securise avec blocage explicite tant que la parite effets preview/export n est pas finalisee
